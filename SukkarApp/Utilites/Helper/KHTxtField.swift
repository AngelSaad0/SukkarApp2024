...
@IBDesignable
open class KHTxtField: UITextField { // swiftlint:disable:this type_body_length
    /**
    ...
    @objc dynamic open var titleFont: UIFont = UIFont(name: "AppRegularFont", size: 14) ?? .systemFont(ofSize: 13) {
        didSet {
            updateTitleLabel()
        }
    }
    ...
    fileprivate final func init_KHTxtField() {
        borderStyle = .none
        createTitleLabel()
        createLineView()
        updateColors()
    ...
    fileprivate func addEditingChangedObserver() {
        self.addTarget(self, action: #selector(KHTxtField.editingChanged), for: .editingChanged)
    }
